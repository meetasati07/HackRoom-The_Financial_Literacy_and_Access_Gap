...
router.get('/money-management', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
        ...
        for (let week = 1; week <= weeksInMonth; week++) {
            ...
            const weekTransactions = transactions.filter(t => t.createdAt >= weekStart && t.createdAt <= weekEnd);
...
router.get('/platform-stats', async (req, res) => {
    try {
        ...
        const totalCoins = await User_1.User.aggregate([
        ...
        const platformStats = {
            ...
            averageRating: 4.8,
            gamesCompleted: Math.floor(totalUsers * 5.5),
...
function getUserLevelMultiplier(level) {
    switch (level) {
