...
// Dynamic weekly trends will be calculated based on actual spending
...
export default function MoneyManagementPage({ user, language, onCoinsUpdate, initialTab = 'categories' }: MoneyManagementPageProps) {
  ...
  // Calculate dynamic weekly trends based on current spending
  const weeklyLimit = monthlyIncome / 4; // Divide monthly income by 4 weeks
  const currentWeekSpending = totalSpent * 0.4; // Assume 40% spent in current week
  const weeklyTrends = [
    { week: 'Week 1', spending: Math.round(totalSpent * 0.2), limit: weeklyLimit },
    { week: 'Week 2', spending: Math.round(totalSpent * 0.25), limit: weeklyLimit },
    { week: 'Week 3', spending: Math.round(totalSpent * 0.35), limit: weeklyLimit },
    { week: 'Week 4', spending: Math.round(currentWeekSpending), limit: weeklyLimit },
  ];

  // Calculate dynamic insights
  const bestWeek = weeklyTrends.reduce((min, week) => week.spending < min.spending ? week : min);
  const peakWeek = weeklyTrends.reduce((max, week) => week.spending > max.spending ? week : max);
  const averageWeeklySpending = Math.round(totalSpent / 4);
                ...
                <CardContent className="pb-6">
                  ...
                  <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    >
                      ...
                      <div className="text-xl font-bold text-green-600">{bestWeek.week}</div>
                      <div className="text-xs text-muted-foreground">₹{bestWeek.spending.toLocaleString()} spent</div>
                    </motion.div>
                    <motion.div 
                    ...
                    >
                      ...
                      <div className="text-xl font-bold text-red-600">{peakWeek.week}</div>
                      <div className="text-xs text-muted-foreground">₹{peakWeek.spending.toLocaleString()} spent</div>
                    </motion.div>
                    <motion.div 
                    ...
                    >
                      ...
                      <div className="text-xl font-bold text-blue-600">₹{averageWeeklySpending.toLocaleString()}</div>
